...
- A genuinely different navigation paradigm for API-backed sources, raised directly and still parked: once you're actually querying an API by date/area, that breaks the tree-structure browsing model the rest of the app is built around — the current Item Set query panel embedded in the tree is a first step, not a final answer to that question.
...
src/
  stac/            framework-agnostic data layer
    ...
    apiSearch.ts      real STAC API /search + rel:items + rel:data (Collections listing) fetching (GET, no
                      backend needed) -- fresh requests optionally carry a datetime/bbox/sortby filter
    conformance.ts    resolves a node's *governing API root's* conformsTo (only ever declared on the root),
                      e.g. to gate the Sort control on whether the server actually supports it
  hooks/
    ...
    useCursorQueriedItemSet.ts API-backed Collections: cursor-following scroll/load-more/load-all, plus a
                               locally-scoped datetime/sort/bbox query (applies only to a fresh request)
  ...
  store/
    ...
    itemSet.ts        what Item Set currently has loaded and visible (no longer "search-filtered" --
                      neither Item Set mode has a text search box; see components/ItemSetBrowser.tsx)
  components/
    ...
    ItemsTimeline.tsx   pure "given items, draw their temporal shape" (grouping, lane packing, axis, optional wheel-zoom/drag-pan) — shared by TimeLens and ItemSetBrowser
    ItemsMap.tsx        pure "given items, draw their footprints" (Leaflet tiles/rectangles/fit-bounds) — shared by SpaceLens and ItemSetBrowser
    DetailPanel.tsx     Inspector (Human/JSON tabs, source + derived facts, embeds TimeLens/SpaceLens)
    ItemSetBrowser.tsx  a thin dispatcher on node.items.kind between two real per-mode panels: static-
                        catalog numbered pagination, or an API-backed datetime/sort/bbox query panel —
                        each with its own List / Time & Space view switcher over the current items
