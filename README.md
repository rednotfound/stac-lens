...
- A dedicated bbox/datetime query-input module living in Item Set's own panel (a different context from the interactive draw-tool an earlier version had directly in Inspector, which was deliberately removed rather than kept alongside its single-object Time/Space widgets). Until it's built, an API-backed Collection is browsable only via its own unfiltered default order.
- Real page-based pagination (jump to page N) in Item Set, replacing scroll-to-load — straightforward for static/links-mode Collections (the full ordered href array is already known), but a real numeric-page jump isn't something a STAC API's own Item Search actually supports (only an opaque `next` cursor/token) — an API-backed jump-to-page can only ever be simulated by sequentially fetching the pages before it.
...
src/
  ...
  components/
    ...
    ItemsTimeline.tsx   pure "given items, draw their temporal shape" (grouping, lane packing, axis) — shared by TimeLens and ItemSetBrowser
    ...
    ItemSetBrowser.tsx  id/title filter + List/Temporal/Spatial view switcher over a Collection's own items
    ...
    TabButton.tsx       the one tab-button style, shared by Inspector's Human/JSON and Item Set's List/Temporal/Spatial
  design/
