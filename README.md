...
src/
  stac/            framework-agnostic data layer
    ...
    apiSearch.ts      real STAC API /search + rel:items + rel:data (Collections listing) fetching (GET, no
                      ...
                      filterToParams() is the single source of truth both the real request and the
                      shareable-URL encoding (searchQueryUrl.ts) build their param names/formats from
  ...
  components/
    ...
    BboxPickerModal.tsx       a full-size, document.body-portaled dialog wrapping ItemsMap in draw mode --
                              opened from the Search box's own "Draw on map" button
